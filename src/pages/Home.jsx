...
const Home = () => {
  ...
  if (loading) {
    return <img className=" flex gap-4 p-4 flex-wrap justify-center  ml-[45%] mt-[10%] w-40 h-40 animate-spin" src="https://www.svgrepo.com/show/199956/loading-loader.svg" alt="Loading icon"/>
  ...
  return (
    ...
    <Navbar/>
      <section className="flex flex-wrap gap-16"> 
        {data.length > 0 ? (
          data.map(product => (
            <div key={product.id} className="w-[350px] pb-2 ml-10 mt-10 border shadow-xl cssanimation fadeInBottom">
              <img className="w-[350px] h-[250px]" src={product.image} alt={product.name} />
              <div className="p-4">
                ...
                <span className="text-xl font-semibold text-primary ml-3">${product.price}</span>
              </div>
              <div className="p-4">
              <Link to={`/product/${product.id}`}>   <button className="bg-accent text-accent-foreground hover:bg-accent/80 py-2 px-4 rounded-lg">
